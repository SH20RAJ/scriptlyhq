...
export default async function CreatorDashboardLayout({
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background text-foreground relative overflow-hidden">
      <CyberBackground />
      <div className="flex-1 flex flex-col md:flex-row max-w-7xl mx-auto w-full px-4 py-8 md:py-12 gap-8 relative z-10">
        
        {/* Left Navigation Sidebar */}
        <aside className="w-full md:w-64 shrink-0 flex flex-col gap-4">
          <div className="p-5 border border-border/40 bg-card/25 backdrop-blur-xl rounded-2xl shadow-sm space-y-3">
          ...
          <div className="border border-border/40 bg-card/25 backdrop-blur-xl rounded-2xl p-4 shadow-sm">
            <SidebarNav />
          </div>
 
          <Link
            href="/dashboard"
            className="flex items-center gap-2 px-4 py-3 rounded-xl border border-border/40 bg-card/25 backdrop-blur-xl text-xs font-bold uppercase tracking-wider text-muted-foreground hover:bg-muted/10 hover:text-foreground transition-all duration-200"
          >
